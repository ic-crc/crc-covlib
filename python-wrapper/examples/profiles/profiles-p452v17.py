...
if __name__ == '__main__':
    ...
    rx_lat = 45.51353
    rx_lon = -75.45785
    sim.SetPropagationModel(covlib.PropagationModel.ITU_R_P_452_V17)
    ...
    sim.SetITURP452HeightGainModelMode(covlib.Terminal.TRANSMITTER, covlib.P452HeightGainModelMode.P452_USE_CLUTTER_PROFILE)
    ...
    csv_pathname = os.path.join(script_dir, 'profiles-p452v17.csv')
    ...
    clutter_category_profile = csv_data['P452-17 clutter category'].tolist()
    itu_radio_clim_zone_profile = csv_data['P452-17 radio climatic zone'].tolist()
    path_loss_profile = csv_data['path loss (dB)'].tolist()

    # get a 'terrain elevation + nominal clutter height' profile
    terr_plus_clut_height_profile = []
    for i in range(len(terr_elev_profile)):
        clutter_height = sim.GetITURP452HeightGainModelClutterValue(covlib.P452HeightGainModelClutterCategory(clutter_category_profile[i]),
                                                                    covlib.P452HeightGainModelClutterParam.P452_NOMINAL_HEIGHT_M)
        terr_plus_clut_height_profile.append(terr_elev_profile[i] + clutter_height)
    ...
    ax1.plot(path_length_km_profile[2:], terr_plus_clut_height_profile[2:], 'g-', label='terrain elevation + nominal clutter height (m)')
